...
| 正式域名 | `riddle-assets.fantasyguide.cn`，待绑定、DNS 与 HTTPS 完成 |
...
用户最终指定域名为 `riddle-assets.fantasyguide.cn`；首次部署记录中的旧拟议地址保留作历史记录，未用于已上线的应用。DNS 目标尚未由控制台生成，不把默认项目域名当作已确认的 CNAME 值。
