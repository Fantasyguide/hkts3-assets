...
托管目标：腾讯云 EdgeOne Pages / Makers，**中国站账号、中国大陆加速区**，域名 `riddle-assets.fantasyguide.cn`。域名需在控制台完成归属、备案、CNAME 与 HTTPS 验证。默认项目预览域名有有效期限制，不作为生产图片地址。当前上线状态见 [STATUS.md](STATUS.md)。

## 免费版容量

2026-09-23 核对的中国站官方配额（配额页更新于 2026-09-22）：

| 项目 | 当前免费版限制 |
| --- | --- |
| 单个静态文件 | 25 MB |
| 单项目文件数 | 20,000 |
| 静态总存储 | 5 GB，同一站点下所有项目合计 |
| 项目数 | 40 |
| 构建次数 | 500 次/月，按套餐周期重置 |
| 并发构建 | 1 |
| 网站加速流量与请求次数 | 当前不限量，仍受平台反滥用机制约束 |

来源：[配额](https://cloud.tencent.com/document/product/1552/132789)、[免费版权益](https://cloud.tencent.com/document/product/1552/127458)。配额可能随商业化调整，不保证永久不变。网页上传与官方项目限制均列出 25 MB 和 20,000 文件；本仓库 SDK 上传同样按此校验，不依赖换上传工具绕开限制。

首批实际只有 23 个公开文件（20 张图片及 3 个托管辅助文件）。图片共 325,708 字节，最大 26,636 字节；50 人各下载一遍共约 16.29 MB 图片正文、1,000 次图片请求，这是算术估计，不是容量压力测试。哈希图片多版本和历史部署会增加占用，5 GB 不能按每个项目独享计算。官方对超过存储限额的建议是清理旧部署，见 [排障说明](https://edgeone.cloud.tencent.com/pages/document/176994654809202688)；活动期间保留必要的回滚版本。
...
当前 `deployment.json` 已绑定首次创建的项目 ID。重新搭建时，将首次工作流摘要里的项目 ID 填入该文件并提交。若项目已存在但没有绑定该 ID，脚本停止，避免误更新同名项目。首次创建成功但上传失败时，日志和制品中的部署记录也会留下 ID；先填写 ID 再重试。
...
```json
"imageBaseUrl": "https://riddle-assets.fantasyguide.cn/"
```
